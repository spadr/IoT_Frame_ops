...
from iot.models import User, TubeModel
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        devices = TubeModel.objects.filter(
            monitoring=True, is_active=True).select_related()
        ...
        for user in users:
            ...
            devices = TubeModel.objects.filter(
                email=user, monitoring=True, is_active=True).order_by('channel', 'name').select_related()
            dead_device = []
            for device in devices:
                ...
                if stopping:
                    dead_device.append(
                        [device.channel, device.name, timediff//60])
